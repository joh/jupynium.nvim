...
if status then
  ...
  local set_last_move = function(move_fn, opts, ...)
    ...
    repeat_move.last_move = { func = move_fn, opts = vim.deepcopy(opts), additional_args = { ... } }
    return true
  end
  ...
  M.goto_next_cell_separator, M.goto_previous_cell_separator =
    make_repeatable_move_pair(M.goto_next_cell_separator, M.goto_previous_cell_separator)
else
  -- master branch compatibility
  status, repeat_move = pcall(require, "nvim-treesitter.textobjects.repeatable_move")
  if status then
    M.goto_next_cell_separator, M.goto_previous_cell_separator =
